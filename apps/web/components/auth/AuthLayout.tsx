        ...
        return (
          <div
            ...
            style={{
              height: isDegraded
                ? "8px"
                : `${8 + Math.sin(i * 0.9) * 4 + parseFloat(seededRandom(i * 17).toFixed(2)) * 3}px`,
            }}
          />
        );
      })}
      ...
      <div className="relative z-10 flex flex-col h-full p-10">
        ...
        <div className="my-auto">
          ...
          <p className="text-zinc-500 text-sm leading-relaxed mb-8 max-w-xs">
            4+ global check locations. Sub-30s alerting. SSL monitoring. Know
          ...
          <div className="space-y-2">
            {UPTIME_DATA.map((m, i) => (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${m.ok ? "bg-emerald-400" : "bg-amber-400 animate-pulse"}`}
                />
                <span className="font-mono text-xs text-zinc-400 flex-1 truncate">
                  {m.name}
                </span>
                <MiniSparkline />
                <span className="font-mono text-[10px] text-zinc-600 w-10 text-right">
                  {m.ms}
                </span>
                <span className="text-[10px] text-zinc-400 font-medium w-12 text-right">
                  {m.uptime}
                </span>
              </div>
            ))}
